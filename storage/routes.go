...
func (s *Storage) GetAllRoutes(ctx context.Context) ([]Route, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, url, target, type, tls, cert, key, enabled, source,
		       allowed_groups, allowed_ips, cookie_policy, renew_on_access, created_at, updated_at
		FROM proxy_routes
		WHERE enabled = TRUE
		ORDER BY url`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&r.Enabled, &r.Source,
			&r.AllowedGroups, &r.AllowedIPs, &r.CookiePolicy, &r.RenewOnAccess,
...
func (s *Storage) UpdateRouteAccess(ctx context.Context, id int, allowedGroups, allowedIPs, cookiePolicy string, renewOnAccess bool) error {
	result, err := s.db.ExecContext(ctx,
		`UPDATE proxy_routes SET allowed_groups = ?, allowed_ips = ?, cookie_policy = ?, renew_on_access = ? WHERE id = ?`,
		allowedGroups, allowedIPs, cookiePolicy, renewOnAccess, id)
	if err != nil {
	...
	slog.Info("route access updated", "id", id, "allowed_groups", allowedGroups, "allowed_ips", allowedIPs)
	return nil
}
...
func (s *Storage) CreateRoute(ctx context.Context, url, target, routeType string) (*Route, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		&r.Enabled, &r.Source, &r.AllowedGroups, &r.AllowedIPs, &r.CookiePolicy, &r.RenewOnAccess,
		&r.CreatedAt, &r.UpdatedAt)
	if err != nil {
